...
def render_jobs_view(components: dict):
    """
    Renders the curated Job Matches tab.
    """
    ...
    # Filter Bar in a clean card container
    with st.container():
    ...
    st.markdown(f"<div style='font-size: 0.85rem; font-weight: 600; color: var(--text-secondary); margin: 16px 0 12px 0;'>Showing top matching openings ({len(filtered)} found):</div>", unsafe_allow_html=True)

    if not filtered:
        st.info("No job openings match the selected filters. Try lowering the minimum match percentage or changing location.")
        return
...
def _render_job_card(job: dict):
    ...
    <div class="job-item-card">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 8px;">
            <div>
                <div style="font-size: 1.15rem; font-weight: 700; color: var(--accent-indigo); margin-bottom: 2px;">
                    {job.get('title', 'Role')}
                </div>
        ...
        <div style="margin-top: 10px; padding-top: 10px; border-top: 0.5px solid var(--border-color);">
            <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 6px; margin-bottom: 4px;">
                <span style="font-size: 0.78rem; font-weight: 700; color: var(--color-success-text); margin-right: 4px;">Skills You Have:</span>
                {have_pills}
            </div>
            <div style="display: flex; flex-wrap: wrap; align-items: center; gap: 6px;">
                <span style="font-size: 0.78rem; font-weight: 700; color: var(--color-danger-text); margin-right: 4px;">Skills Missing:</span>
                {missing_pills}
            </div>
        </div>
    </div>
