...
def render_ats_view(components: dict, ats_results: dict):
    """
    Renders the hero ATS score evaluation screen.
    """
    ...
    with col_hero_ring:
        ...
        <div class="hero-score-card">
            <div style="font-size: 0.85rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-secondary); margin-bottom: 8px;">
                ATS Screening Score
            </div>
            <div style="position: relative; width: 170px; height: 170px; display: flex; align-items: center; justify-content: center; margin: 4px 0 12px 0;">
                ...
                <div style="position: absolute; text-align: center;">
                    <div style="font-size: 2.5rem; font-weight: 800; color: {ring_color}; line-height: 1;">
                        {total_score}
                    </div>
            ...
            <div style="font-size: 0.88rem; font-weight: 600; color: {ring_color}; background: {bg_ring_color}; padding: 6px 14px; border-radius: 999px; max-width: 90%;">
                ● {verdict}
            </div>
            <div style="font-size: 0.75rem; color: var(--text-muted); margin-top: 8px;">
                Evaluated against {target_role_title} industry benchmarks
            </div>
        </div>
    ...
    with col_subscores:
        ...
        st.markdown('<h4 style="margin-top: 0; margin-bottom: 16px; font-weight: 700;">Score Breakdown & Metrics</h4>', unsafe_allow_html=True)
        ...
        <div class="subscore-row" style="margin-top: 10px;">
            <div class="subscore-header">
                ...
                <span>{int(exp_score)}%</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        st.progress(min(1.0, exp_score / 100.0))

        st.markdown("<hr style='margin: 16px 0; border: none; border-top: 0.5px solid var(--border-color);'>", unsafe_allow_html=True)
    ...
    with col_wins:
        ...
        <div class="cie-card">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px;">
                <span style="background: var(--color-success-bg); color: var(--color-success); padding: 4px 8px; border-radius: 6px; font-weight: 700;">✓</span>
                <span style="font-size: 1.05rem; font-weight: 700; color: var(--color-success-text);">What's Working (Strengths)</span>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_issues:
        ...
        <div class="cie-card">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px;">
                <span style="background: var(--color-danger-bg); color: var(--color-danger); padding: 4px 8px; border-radius: 6px; font-weight: 700;">!</span>
                <span style="font-size: 1.05rem; font-weight: 700; color: var(--color-danger-text);">What to Fix (Actionable Fixes)</span>
            </div>
        """, unsafe_allow_html=True)
