...
Accessible in both Light Mode and Dark Mode with native Streamlit containers.
"""
...
def render_market_view(components: dict):
    """
    Renders the Market Intelligence tab with robust error handling and native containers.
    """
    try:
        ...
        <div style="margin-bottom: 16px;">
            <h3 style="margin: 0 0 4px 0; font-weight: 800; color: var(--text-primary);">📈 Real-Time Job Market Intelligence</h3>
        ...
        with col1:
            with st.container(border=True):
                st.markdown(f'<h4 style="margin-top:0; font-weight:800; color: var(--text-primary);">🏆 Top In-Demand Skills for {role_title}</h4>', unsafe_allow_html=True)
                try:
        ...
        with col2:
            with st.container(border=True):
                st.markdown('<h4 style="margin-top:0; font-weight:800; color: var(--text-primary);">💰 Compensation Benchmarks Across Tech Roles</h4>', unsafe_allow_html=True)
                try:
        ...
        with c_loc1:
            with st.container(border=True):
                st.markdown('<h4 style="margin-top:0; font-weight:800; color: var(--text-primary);">🌍 Geographic Hiring Hub Distribution</h4>', unsafe_allow_html=True)
                try:
        ...
        with c_loc2:
            with st.container(border=True):
                st.markdown('<h4 style="margin-top:0; font-weight:800; color: var(--text-primary);">🏢 Top Tech Employers Hiring</h4>', unsafe_allow_html=True)
                try:
                    overview = market_analyzer.get_market_overview()
