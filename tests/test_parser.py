...
def test_xxe_vulnerability_in_docx(parser):
    """Ensure XXE vulnerabilities are mitigated in DOCX parsing."""
    import io
    import zipfile
    import defusedxml

    xxe_payload = b"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE foo [<!ENTITY xxe "malicious_entity">]>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    ...
    with zipfile.ZipFile(buf, "w") as z:
        z.writestr("word/document.xml", xxe_payload)

    malicious_bytes = buf.getvalue()

    # We expect our parser to safely handle it (either catch the exception or bubble it up,
    # but the parsing itself should throw defusedxml.common.EntitiesForbidden)
    # The parser currently catches Exception and logs a warning for XML zip extraction, returning None for that block.
    # Let's ensure the defusedxml exception is raised internally.
    with pytest.raises(defusedxml.common.EntitiesForbidden):
        # We simulate the exact internal call since parser.parse() catches the Exception
        # and falls back, but let's test if we can observe it or just check parse result.
        # Since parser.parse catches it:
        # try:
        #     tree = ET.fromstring(xml_content)
        # except Exception as e:
        #     logger.warning(f"DOCX XML zip extraction warning: {e}")

        # We can bypass the top-level parse and test the ET directly on this payload,
        # or we can test parser._extract_text which handles this logic.
        # Actually, let's just trigger it directly through defusedxml as used by parser:
        from src.nlp.parser import ET as parser_ET
        parser_ET.fromstring(xxe_payload)

    # Let's also verify that parsing via the parser doesn't result in extracting "malicious_entity"
    result = parser.parse(malicious_bytes, filename="resume.docx")
    # Even if it succeeds somehow (fallback), it shouldn't contain the entity
    profile = result.get("profile", {})
    text = profile.get("raw_text", "")
    assert "malicious_entity" not in text
